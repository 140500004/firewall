        ...
        <div id="listaRegras" class="tab-pane fade in active">
            ...
            @else
                ...
                @foreach($RegrasG as $rg)
                    {{ Form::model('regras', ['method' => 'PUT','route' => ['regras.update', $rg->id_regras],'class' => 'conf-update'])}}
                    <div class="input-group">
                        {{ Form::label('www', 'www.', array('class' => 'input-group-addon')) }}
                        ...
                        {{ Form::open(['method' => 'DELETE','route' => ['regras.destroy', $rg->id_regras],'style'=>'display:inline', 'class' => 'conf-drop']) }} <!-- 'id' => 'conf' Não funciona-->
